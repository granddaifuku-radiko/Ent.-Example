...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAdministered; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Administered = []*Category{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			fk := n.category_admin
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_admin" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_admin" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Administered = append(node.Edges.Administered, n)
		}
	}
